...
| 4 | Behavioral / gate layer | 🟡 started (NAND-game seed) | Truth-table extraction from grouped circuits over the real S-matrix sim (#934), Truth Table panel with threshold + raw powers (#947), the build→group→extract→save/load E2E journey (#952), and the shipped logic-gate starter example (#958) are merged. Next: transfer-function models, delays, logic-level animation. |
