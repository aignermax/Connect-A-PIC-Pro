...
- Rung 1 remainder: #880 (opt-in auto-connect after import); groundwork still open: #704 — split into the independently fixable #1077 (A* self-loop), #1078 (unchecked terminal approach overlaps a neighbour waveguide), #1079 (stale crossing dissolution) — and #725 (routing rework: direct-first, anytime-A*)
- Rung 4/5 next: #1071 (signal-name collision hint), #1070 (1-bit ALU example, in progress); transfer functions and sequential logic (latches) remain unscoped
...
- Onboarding (principle 1): #1048 (one-click Examples) and #768 (examples in release bundles) shipped; #769 (first-steps tutorial) — slice 1 scoped as #1080 (step engine + place→connect→simulate)
