...
{
    /// <summary>
    /// Progress of a Monte-Carlo run: how many of the jittered runs finished.
    /// </summary>
    /// <param name="CompletedRuns">Number of jittered runs completed so far.</param>
    /// <param name="TotalRuns">Total number of jittered runs requested.</param>
    ...
    /// curve, then N runs with Gaussian jitter applied to the given sliders.
    /// The metric is supplied as a delegate so the same runner serves spectrum
    /// envelopes, eye-openness distributions, or any other per-run curve.
    /// Runs are sequential because the jitter mutates shared slider state;
    /// the sampler delegate may offload its own computation to worker threads.
    /// Slider values are always restored to nominal, even on cancel or error.
    /// </summary>
    public class MonteCarloRunner
    {
        ...
        /// Runs the analysis.
        /// </summary>
        /// <param name="configuration">Run count, sigma and seed.</param>
        /// <param name="sliders">Sliders to jitter (all parameters of the design under test).</param>
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                for (int run = 0; run < configuration.RunCount; run++)
                {
                    ...
                    jitter.ApplyJitter(sampler, configuration.SigmaRelative);
